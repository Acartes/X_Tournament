...
public class MoveBehaviour : NetworkBehaviour
{
  ...
  [ReadOnly] public List<GameObject> GoPathes;
  // déplacement
    ...
    foreach (Transform path in this.pathes)
      {
        ...
        path.GetComponent<CaseData>().ChangeStatut(Statut.None, Statut.canMove);
        path.GetComponent<CaseData>().ChangeStatut(Statut.None, Statut.canBeTackled);
      }
